  ...
  async sessionsValidateAuth(reqHeader) {
    try {
      ...
      if (!authHeader) {
        ...
        // Uso do split para separar e pegar só o TOKEN
        ...
        // Por padrão a função jwt não retorna uma promise, usar promisify.
        // Decoded: objeto apenas cm o id do usuário.
        const decoded = await promisify(jwt.verify)(token, configAuth.secret);
        utils.devLog(2, 'API ==> Controller => Decoded', null);
        utils.devLog(2, decoded, null);

        // Nesse middlweare pra frente vai ter info qual id do usuário está fazendo a req.
