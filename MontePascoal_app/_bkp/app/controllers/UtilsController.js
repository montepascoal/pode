  ...
  utilsGetDataCnpj = async ({body,params}, res) => {
    ...
    try {
      ...
      if (!validateParameters(cnpj)) {
        return utils.resError(400,`API ==> Controller => utilsGetDataCnpj -> Invalid parameters`, null, res);
      }
      
      const resGetCnpj = await apiREST.get('https://www.receitaws.com.br/v1/cnpj/'+objData.cnpj);
      LogsController.logsCreate(useId, perId, logMsg, `=> [${useId}] # Config: Busca CNPJ ## [geral]`, null);
      return utils.resSuccess('API ==> Controller => utilsGetDataCnpj -> Success',resGetCnpj.data, res);    

    } catch (error) {
      return utils.resError(500,`API ==> Controller => utilsGetDataCnpj -> Error`, error, res);
    }
