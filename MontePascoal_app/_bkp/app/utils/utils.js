...
const functions = {
  ...
  logTXT( type, msg, obj ) { 
    ...
    const strFile = path.resolve(__dirname, '..', '..', 'logs')+`/${auxDevDateNowToStr()}.txt`;
    fs.open(strFile, 'a', 666, function( e, id ) {
      fs.write( id, strLOG + "\n", null, 'utf8', function() {
        fs.close(id, function() {
          // console.log('file is updated');
          // console.log('====================================================================')
          // console.log(path.resolve(__dirname, '..', '..', 'logs')+`/${auxDevDateNowToStr()}.txt`);
          // console.log('/mnt/c/projects/systemFaciliteCard/app/logs/log.txt');
        });
      });
    });
  },
...
const auxDevLog = (msg, obj) => {
  console.log("■", auxDevTimeNowToStr(), "==>", msg ? msg : '');
  obj ? console.log(obj) : null;
};
const auxDevError = (msg, error) => {
    error ? console.error("♦", auxDevTimeNowToStr(), "==> Description:") : null;
    error ? console.log('---------------------') : null;
    error ? console.error(error) : null;
    error ? console.log('---------------------') : null;
    console.error("♦", auxDevTimeNowToStr(), "==>", msg);
  // alert(msg);
};
...
const auxDevNumTresDigitos = (num) => {
  if (num < 10) {
    ...
    // Mais de quatro digitos
    return num.toString().substr(0, 3);
  }
};
const auxDevTimeNowToStr = () => {
